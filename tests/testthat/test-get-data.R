...
test_that("get_data produces expected output for documents endpoint", {
  ...
  url <- paste0(
    "https://api.regulations.gov/v4/documents?filter[docketId]=CMS-2014-0063&api_key=",
    key
  )
  res <- get_data(url)
  ...
  url <- paste0(
    "https://api.regulations.gov/v4/documents?filter[docketId]=CMS-2014-0063&include=attachments&api_key=",
    key
  )
  res_df <- get_data(url,
    df = TRUE
  ...
  url <- paste0(
    "https://api.regulations.gov/v4/documents?filter[documentType]=Supporting%20%26%20Related%20Material,Notice&filter[searchTerm]=benzene,runoff&page[number]=1&page[size]=250&api_key=",
    key
  )
...
test_that("get_data produces expected output for comments endpoint", {
  ...
  url <- paste0(
    "https://api.regulations.gov/v4/comments?filter[docketId]=CMS-2014-0063&api_key=",
    key
  )
  res <- get_data(url)
...
test_that("get_data throws error when invalid url provided", {
  ...
  url <- paste0(
    "https://api.regulations.gov/v4/comments/FDA-2012-n-0920-0001?include=attachments&api_key=",
    key
  )
  expect_error(
    get_data(url),
