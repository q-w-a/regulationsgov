...
test_that("data frame is correct for full example", {
  ...
  result <- get_all_comments(endpoint = "document",
...
test_that("quiet works and expected message is provided", {
  skip_if_no_key()
  expect_message( get_all_comments(endpoint = "document",
...
test_that("works for dockets endpoint", {
  skip_if_no_key()
  url <- construct_docket_url(agencyId = c("FDA"),
                              lastModifiedDate = c("2021-02-22 12:00:00",
                                                   "2021-02-26 12:00:00"))

  comments <- get_all_comments(endpoint = "docket",
                               agencyId = c("FDA"),
                               lastModifiedDate = c("2021-02-22 12:00:00",
                                                    "2021-02-26 12:00:00"))
...
test_that("throws error for invalid endpoint", {
  skip_if_no_key()
  expect_error(get_all_comments(endpoint = "Not"),
